...
- **The undo token reaches surfaces we do not control.** `publicView` hands `undo.token` to the model, because `commit_change` requires it — the token is the nonce proving verification happened, so the model has to carry it. The consequence is that any UI rendering tool arguments will display it. Our console redacts, and `tools/mcp-call.mjs` redacts its terminal output, but **TrueForge's own turn view renders the raw arguments** and its API serves the real value; that surface cannot be fixed from this repo. Do not put TrueForge's turn view on camera. The architectural fix — have the engine bind the token to the simulation server-side so the model never holds it — is a post-submission change, not a demo-day one.
