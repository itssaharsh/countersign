...
**Screen.** The Countersign console, cold. One column, the claim centred in the frame the
header and the gate bar leave:
*Every approval gate shows you the command. This one shows you the consequence.* Let it sit
for two full seconds before you speak.
...
**Do not move the mouse during this beat.** The screen is the argument.

---

## Beat 3 · The order — 0:32 to 0:52

**Screen.** Click the field, paste the order, press **Measure it**. The layout splits: the
transcript arrives on the left, the dossier on the right, the phase track lights INVESTIGATING.
...
**Screen.** The ledger stamps in complete. Rows count up by foreign-key depth. The total turns
...
**Screen.** The three precondition lines, stamped. Then the countersign control materialises
in the gate bar — the red arriving after the shape.
...
**Screen.** Press and hold. The fill crosses the label, left to right, 1200ms. Release only
once it reads COUNTERSIGNED. The gate clears, the ground shifts, the receipt prints.
...
- **Do not let it add zooms to beat 2 or beat 4.** Those two screens are compositions; a
  push-in crops the claim and the ledger total is the wrong shape for a zoom.
